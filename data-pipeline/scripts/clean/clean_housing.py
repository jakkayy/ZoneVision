...
def main():
    ...
    # ตรวจสอบว่าไฟล์เป็นแบบภาษาไทยดิบ หรือถูกแมปชื่อภาษาอังกฤษแล้ว
    if 'district' in df.columns:
        df_clean = df.copy()
        df_clean['district'] = df_clean['district'].astype(str).str.replace('ท้องถิ่นเขต', 'เขต').str.strip()
    else:
        target_features = {
    ...
    num_cols = ['house', 'townhouse', 'semi_detached_house', 'condo', 'flat', 'dormitory', 'office', 'shop', 'shophouse', 'total_buildings']
    for col in num_cols:
